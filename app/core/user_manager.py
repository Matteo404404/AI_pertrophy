...
class UserManager:
    ...
    def get_user_ml_profile(self, user_id):
        ...
        """
        tier_progress = self.db.get_user_tier_progress(user_id)
        current_tier = tier_progress['current_tier'] # 0, 1, or 2
        
        # Base knowledge starts at 0.2 (novice) and caps at 0.95 (expert)
        base_score = 0.2 + (current_tier * 0.25)
        
        # Calculate specific domain scores based on wrong answers in DB
        # If they missed many "Sleep" questions, their recovery_knowledge is lower
        learning_data = self.get_user_learning_data()
        mistakes = learning_data['grouped_mistakes']
        
        recovery_penalty = len(mistakes.get('Recovery & Sleep', [])) * 0.05
        volume_penalty = len(mistakes.get('Training Volume', [])) * 0.05
        technique_penalty = len(mistakes.get('Training Intensity', [])) * 0.05
        
        return {
            'training_literacy_index': max(0.1, base_score),
            'recovery_knowledge': max(0.1, base_score - recovery_penalty),
            'load_management_score': max(0.1, base_score - volume_penalty),
            'technique_score': max(0.1, base_score - technique_penalty)
        }
